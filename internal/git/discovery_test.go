...
func TestDiscover(t *testing.T) {
	...
	os.MkdirAll(filepath.Join(stdRepo, ".git"), 0755)

	// 2. Bare repo
	bareRepo := filepath.Join(tmpDir, "bare-repo")
	os.MkdirAll(bareRepo, 0755)
	os.WriteFile(filepath.Join(bareRepo, "config"), []byte(""), 0644)
	os.MkdirAll(filepath.Join(bareRepo, "refs"), 0755)
	os.MkdirAll(filepath.Join(bareRepo, "objects"), 0755)
	os.WriteFile(filepath.Join(bareRepo, "HEAD"), []byte(""), 0644)

	// 3. Worktree (should be ignored)
	wtRepo := filepath.Join(tmpDir, "worktree-repo")
	os.MkdirAll(wtRepo, 0755)
	os.WriteFile(filepath.Join(wtRepo, ".git"), []byte("gitdir: ..."), 0644)

	// 4. Nested repo
	nestedParent := filepath.Join(tmpDir, "nested")
	os.MkdirAll(nestedParent, 0755)
	...
	foundNested := false
	for _, r := range repos {
		if r.LocalPath == stdRepo { foundStd = true }
		if r.LocalPath == bareRepo { foundBare = true }
		if r.LocalPath == nestedRepo { foundNested = true }
		if r.LocalPath == wtRepo {
			t.Errorf("found worktree repo %s, which should be ignored", wtRepo)
		}
	}

	if !foundStd { t.Error("did not find standard repo") }
	if !foundBare { t.Error("did not find bare repo") }
	if !foundNested { t.Error("did not find nested repo") }
}
